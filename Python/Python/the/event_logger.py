...
class EventLogger:
    ...
    def _initialize(self):
        # Ensure DB path is absolute and in project root
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        PROJECT_ROOT = os.path.dirname(BASE_DIR)
        self.db_path = os.path.join(PROJECT_ROOT, "trading_bot_audit.db")
    ...
    def log_signal(self, sig: dict):
        ...
        reason = sig.get('reason', 'Signal generated by v2.0 engine')
        readable_msg = f"The intelligence engine identified a potential {sig['signal_type']} opportunity for {sig['symbol']} at {sig['price']:.2f}. Reasoning: {reason}."
        ...
        params = (datetime.now().isoformat(), sig['symbol'], sig['signal_type'], sig['confidence'], sig['regime'], reason, json.dumps(sig))
    ...
    def log_trade_entry(self, trade: dict):
        ...
        params = (trade['trade_id'], trade['symbol'], trade['direction'], trade['quantity'], trade['entry_price'], "OPEN", trade['timestamp'], mode, signal_id)
        self.log_queue.put((query, params))
